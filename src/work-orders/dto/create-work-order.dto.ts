...
class WorkOrderItemDto {
  @ApiProperty({ 
    ...
    description: 'Nombre del servicio realizado (ver /work-orders/services-catalog)' 
  })
  @IsString()
  ...
  @ApiPropertyOptional({ 
    example: 'F-001', 
    description: 'SKU del producto usado (si aplica). Descuenta stock automáticamente.' 
  })
  @IsString()
  @IsOptional()
  product_sku?: string;
...
class ClientDto {
  @ApiProperty({ example: 'Juan Pérez González', description: 'Nombre completo del cliente' })
  @IsString()
  nombre: string;
  ...
  @ApiPropertyOptional({ example: 'juan.perez@gmail.com', description: 'Email del cliente' })
  @IsEmail()
  @IsOptional()
  email: string;
...
class VehicleDto {
  ...
  @ApiPropertyOptional({ example: 85000, description: 'Kilometraje actual del vehículo' })
  @IsInt()
  @IsOptional()
  kilometraje: number;
}
...
export class CreateWorkOrderDto {
  @ApiProperty({ 
    example: 1547, 
    description: 'Número del talón físico (correlativo del talonario rojo). Debe ser ÚNICO.' 
  })
  @IsInt()
  numero_orden_papel: number;

  @ApiProperty({ example: 'Carlos González', description: 'Nombre del mecánico que realizó el trabajo' })
  @IsString()
  realizado_por: string;

  @ApiPropertyOptional({ example: 'Pedro Supervisor', description: 'Nombre de quien supervisó/revisó el trabajo' })
  @IsString()
  @IsOptional()
  revisado_por: string;

  @ApiProperty({ 
    description: 'Datos del cliente. Si el RUT ya existe, se actualiza el registro.',
    type: ClientDto 
  })
  @ValidateNested()
  @Type(() => ClientDto)
  ...
  @ApiProperty({ 
    description: 'Lista de servicios realizados. Cada item puede tener un producto asociado.',
